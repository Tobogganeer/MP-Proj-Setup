...
namespace VirtualVoid.Net
{
    public class VoiceChatManager : MonoBehaviour
    {
        ...
        {
            SteamManager.AddInternalHandler_FromClient(InternalClientMessageIDs.SEND_VOICE, OnClientVoice);
            SteamManager.AddInternalHandler_FromServer(InternalServerMessageIDs.BOUNCE_VOICE, OnServerVoice);
        }

        private void OnDisable()
        {
            SteamManager.RemoveInternalHandler_FromClient(InternalClientMessageIDs.SEND_VOICE, OnClientVoice);
            SteamManager.RemoveInternalHandler_FromServer(InternalServerMessageIDs.BOUNCE_VOICE, OnServerVoice);
        }
        ...
        private void Update()
        {
            ...
            bool record = Input.GetKey(microphoneActivationKeycode) &&
                SteamManager.ConnectedToServer;

            if (record != oldRecord)
            {
                if (microphoneRecordingIcon != null)
                    microphoneRecordingIcon.SetActive(record);
